...
_MIN_TEXT_LEN = 8
# Лимит на максимум тем в одном сообщении. Снижено до 2: при 3 модель
# начинала "добивать" слот случайным похожим slug'ом ("почему так грубо?"
# → [оплата, скидка, возврат_средств]). Одна точная тема ценнее трёх размытых.
_MAX_TAGS_PER_MESSAGE = 2
...
_PROMPT_TEMPLATE = (
    "Ты строгий классификатор тем для обращений клиентов в поддержку.\n"
    "Доступные темы (slug'и):\n{vocab}\n\n"
    "ГЛАВНОЕ ПРАВИЛО: лучше ответить `none`, чем поставить неточный тег.\n"
    "Дефолт — `none`. Ставь тему ТОЛЬКО если она прямо явствует из текста.\n\n"
    "Правила:\n"
    "- 0, 1 или максимум 2 темы. Одна точная лучше двух приблизительных.\n"
    "- Если сомневаешься хоть в одной теме — отвечай `none`.\n"
    "- НЕ угадывай по отдельным словам ('грубо' ≠ оплата, 'дорого' ≠ скидка).\n"
    "- НЕ ставь синонимы вместе (например, не дублируй похожие темы).\n"
    "- Прощание ('пока', 'до свидания', 'спасибо, всё') → `none`.\n"
    "- Эмоция/жалоба без конкретики ('почему так грубо?', 'это ужасно') "
    "→ `жалоба` или `качество_обслуживания`, НЕ оплата/доставка/возврат.\n"
    "- Короткие реплики без сути ('ок', 'ясно', 'хорошо') → `none`.\n\n"
    "Примеры:\n"
    "Текст: 'почему так грубо разговариваете?' → жалоба,качество_обслуживания\n"
    "Текст: 'ладно, до свидания' → none\n"
    "Текст: 'когда привезёте заказ №123?' → статус_заказа,срок_доставки\n"
    "Текст: 'не приходит код в смс при входе' → доступ_в_личный_кабинет\n"
    "Текст: 'спасибо большое за помощь!' → none\n"
    "Текст: 'хочу вернуть деньги за товар' → возврат_средств\n\n"
    "Формат ответа: только slug'и через запятую (или `none`). "
    "Никаких комментариев, заголовков, кавычек, пояснений."
)
